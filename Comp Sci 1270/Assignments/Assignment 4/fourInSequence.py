...
def checkDraw(board):
    ...
    for row in range(0, len(board)):
        for column in range(0, len(board[0])):
            if board[row][column] == empty and board[row][column+1] == empty and board[row][column+2] == empty and board[row][column+3] == empty and board[row][column+4] == empty and board[row][column+5] == empty and board[row][column+6] == empty:
                return True
            else:
                return False
...
def checkWinner(board, playerNumber):
    ...
    for column in range(0, len(board)):
        for row in range(0, len(board[0]) - 3):
            if board[column][row] == piece and board[column][row+1] == piece and board[column][row+2] == piece and board[column][row+3] == piece:
                return True
...
def main():
    ...
    while running:
        ...
        if choice == "p":
            ...
            print("Choose the Column to drop your piece into!")
            print("If playing against the computer outsmart it and try to get four pieces in a row")
        elif choice == "q":
